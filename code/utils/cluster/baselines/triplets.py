...
from code.utils.cluster.data import (
    _cifar100_to_cifar20,
    _create_dataloaders,
    _create_mapping_loader,
)
...
def make_triplets_data(config):
    ...
    shuffled_dataloaders = _create_dataloaders(
        config,
        dataset_class,
        tf1,
        tf2,
    ...
    dataloader_test = _create_mapping_loader(
        config,
        dataset_class,
        tf3,
...
def triplets_get_data(config, net, dataloader, sobel):
    ...
    flat_targets_all = torch.zeros(
        (num_batches * config.batch_sz), dtype=torch.int32
    ).cuda()
    flat_preds_all = torch.zeros(
        (num_batches * config.batch_sz), dtype=torch.int32
    ).cuda()
...
def triplets_get_data_kmeans_on_features(config, net, dataloader, sobel):
    ...
    features_all = np.zeros(
        (num_batches * config.batch_sz, config.output_k), dtype=np.float32
    )
...
def triplets_eval(config, net, dataloader_test, sobel):
    ...
    match = _hungarian_match(
        flat_preds_all, flat_targets_all, preds_k=config.gt_k, targets_k=config.gt_k
    )

    found = torch.zeros(config.gt_k)  # sanity
    reordered_preds = torch.zeros(num_samples, dtype=flat_preds_all.dtype).cuda()
...
def triplets_loss(outs_orig, outs_pos, outs_neg):
    ...
    # loss is minimised
    return F.kl_div(orig, pos, reduction="elementwise_mean") - F.kl_div(
        orig, neg, reduction="elementwise_mean"
    )
