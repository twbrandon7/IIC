...
def save_progress(
    ...
    flat_predss_all, flat_targets_all, soft_predss_all = _clustering_get_data(
        config,
        net,
        mapping_test_dataloader,
        sobel=sobel,
    ...
    reordered_soft_preds = torch.zeros(
        (num_samples, config.gt_k), dtype=soft_preds.dtype
    ).cuda()
    for pred_i, target_i in match:
        reordered_soft_preds[:, GT_TO_ORDER[target_i]] += soft_preds[
            :, pred_i
        ]  # 1-1 for IIC
    ...
    best_colours = [
        list((np.array(hsv_to_rgb(hue, 0.8, 0.8)) * 255.0).astype(np.uint8))
        for hue in hues
    ]
    ...
    for colour_i, colours in enumerate(all_colours):
        ...
        image = (
            np.ones((2 * (scale + border), 2 * (scale + border), 3), dtype=np.uint8)
            * 255
        )
        ...
        averaged_imgs = [
            np.zeros((config.input_sz, config.input_sz, 1)) for _ in range(config.gt_k)
        ]
        averaged_imgs_norm = [0.0 for _ in range(config.gt_k)]
        counter = 0
        ...
        for c in range(config.gt_k):
            ...
            averaged_img = averaged_img.resize(
                (average_side, average_side), Image.BILINEAR
            )
