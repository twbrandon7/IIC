...
def train_kmeans(config, net, test_dataloader):
    ...
    features_all = np.zeros(
        (config.max_num_kmeans_samples, net.module.features_sz), dtype=np.float32
    )
    ...
    for i, tup in enumerate(test_dataloader):
        ...
        if config.verbose and i < 2:
            print(
                "(kmeans_segmentation_eval) through model %d time %s"
                % (i, datetime.now())
            )
            sysout.flush()
        ...
        if config.verbose and i < 2:
            print(
                "(kmeans_segmentation_eval) applied mask %d time %s"
                % (i, datetime.now())
            )
            sysout.flush()
        ...
        if config.verbose and i < 2:
            print(
                "(kmeans_segmentation_eval) applied select %d time %s"
                % (i, datetime.now())
            )
            sysout.flush()
        ...
        if config.verbose and i < 2:
            print("(kmeans_segmentation_eval) stored %d time %s" % (i, datetime.now()))
            sysout.flush()
...
def apply_trained_kmeans(config, net, test_dataloader, kmeans):
    ...
    for i, tup in enumerate(test_dataloader):
        ...
        imgs, mask_cuda, targets, mask_np = (
            imgs.cuda(),
            mask.cuda(),
            targets.cuda(),
        ...
        targets_all[actual_num_unmasked : actual_num_unmasked + num_unmasked] = targets
    ...
    match = _hungarian_match(
        preds_all, targets_all, preds_k=config.gt_k, targets_k=config.gt_k
    )
    torch.cuda.empty_cache()
    ...
    if GET_NMI_ARI:
        nmi, ari = _nmi(reordered_preds, targets_all), _ari(
            reordered_preds, targets_all
        )
    else:
        nmi, ari = -1.0, -1.0
    ...
    for c in range(config.gt_k):
        reordered_masses[c] = float((reordered_preds == c).sum()) / actual_num_unmasked
