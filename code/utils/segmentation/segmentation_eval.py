...
def _segmentation_get_data(
    config, net, dataloader, sobel=False, using_IR=False, verbose=0
):
    ...
    for b_i, batch in enumerate(dataloader):
        ...
        for i in range(config.num_sub_heads):
            ...
            flat_predss_all[i][start_i : (start_i + actual_samples_curr)] = (
                flat_preds_curr.view(-1)
            )

        flat_targets_all[start_i : (start_i + actual_samples_curr)] = flat_targets.view(
            -1
        )
        ...
        if verbose > 0 and b_i < 3:
            ...
            print(
                "finished batch %d, %s, took %s, of %d"
    ...
    if verbose > 0:
        ...
        print("finished all batches %s, took %s" % (all_finish, all_finish - all_start))
        sys.stdout.flush()
    ...
    assert len(flat_predss_all[0].shape) == 1 and len(flat_targets_all.shape) == 1
