...
for model_ind in model_inds:
    ...
    for imgs_dataloader_name in args.imgs_dataloaders:
        for net_name in net_names:
            ...
            print("starting to run test data through for rendering %s" % datetime.now())
            all_matches, all_accs = _get_assignment_data_matches(
                net,
                mapping_assignment_dataloader,
                config,
                sobel=(not config.no_sobel),
            ...
            colour_map_raw = [
                (np.random.rand(3) * 255.0).astype(np.uint8)
            ...
            if "Coco" in config.dataset:
                colour_map_gt = [
                    np.array([39, 159, 216], dtype=np.uint8),
            ...
            for b_i, batch in enumerate(imgs_dataloader):
                ...
                if not config.no_sobel:
                    imgs = sobel_process(
                        orig_imgs, config.include_rgb, using_IR=config.using_IR
                    )
                else:
                    imgs = orig_imgs
                ...
                reordered_preds = np.zeros(
                    (num_imgs_curr, h, w), dtype=flat_targets.dtype
                )
                ...
                if "Coco" in config.dataset:
                    ...
                    orig_imgs_swapped = torch.zeros(
                        orig_imgs.shape, dtype=orig_imgs.dtype
                    )
                    orig_imgs_swapped[:, 0, :, :] = orig_imgs[:, 2, :, :]
                    orig_imgs_swapped[:, 1, :, :] = orig_imgs[:, 1, :, :]
                    orig_imgs_swapped[:, 2, :, :] = orig_imgs[
                        :, 0, :, :
                    ]  # ignore others
                    render(
                        orig_imgs_swapped,
                        mode="image",
                        name=("%d_img" % model_ind),
                        offset=next_img_ind,
            ...
            for c in range(config.gt_k):
                ...
                print(
                    "class %d: name %s: pred %d correct %d all %d %f iou"
                    % (
                        c,
                        all_label_names[c],
                        predicted_all[c],
