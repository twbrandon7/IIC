...
def main():
    ...
    if config.gt_k == 3:
        ...
        colour_map = [
            (np.random.rand(3) * 255.0).astype(np.uint8) for _ in range(config.gt_k)
        ]
    ...
    input_blocks = np.zeros(
        (NUM_TRAIN, 4, config.input_sz, config.input_sz), dtype=np.uint8
    )
...
def split_imgs(
    ...
    img = cv2.resize(
        img, dsize=None, fx=rescale, fy=rescale, interpolation=cv2.INTER_LINEAR
    )

    img = cv2.resize(
        img, dsize=None, fx=rerescale, fy=rerescale, interpolation=cv2.INTER_LINEAR
    )  # 1500, 1500 or 3000, 3000
...
def filter_gt_and_store(config, img_ind, gt, rescale, rerescale, colour_map, out_dir):
    ...
    # rescale
    recoloured = cv2.resize(
        recoloured, dsize=None, fx=rescale, fy=rescale, interpolation=cv2.INTER_NEAREST
    )
    recoloured = cv2.resize(
        recoloured,
        dsize=None,
...
def predict_and_reassemble(
    config, input_blocks, num_big_imgs, save_names_to_orig_pos, colour_map, out_dir
):
    ...
    for b_i in range(num_batches):
        ...
        imgs = torch.from_numpy(
            input_blocks[start:end_excl, :, :, :].astype(np.float32) / 255.0
        )
    ...
    output_imgs = np.zeros((num_big_imgs, output_h_w, output_h_w, 3), dtype=np.uint8)
    ...
    for name in range(num_imgs):
        ...
        output_imgs[
            output_img_i,
            output_h_i : (output_h_i + trimmed_h_w),
