...
parser.add_argument(
    "--doersch_stats",
    type=str,
    default="/users/xuji/iid/iid_private/code/datasets" "/segmentation/baselines",
)
...
for e_i in range(next_epoch, config.num_epochs):
    ...
    for b_i, tup in enumerate(dataloader):
        ...
        centre, other, position_gt = doersch_set_patches(
            input_sz=config.input_sz, patch_side=config.doersch_patch_side
        )
