...
from code.utils.segmentation.data import make_Coco_dataloaders, make_Potsdam_dataloaders
from code.utils.segmentation.baselines.kmeans_segmentation_eval import (
    kmeans_segmentation_eval,
)
from code.utils.segmentation.baselines.isola_utils import isola_loss, isola_set_patches
...
# COCO and Potsdam
parser.add_argument("--use_coarse_labels", default=False, action="store_true")  # new

# for COCO only
parser.add_argument(
    "--include_things_labels", default=False, action="store_true"
)  # new
parser.add_argument("--incl_animal_things", default=False, action="store_true")  # new
...
if "Coco10k" in config.dataset:
    ...
    config.train_partitions = ["unlabelled_train", "labelled_train", "labelled_test"]
...
if "Coco" in config.dataset:
    dataloaders, mapping_assignment_test_dataloader, _ = make_Coco_dataloaders(config)
elif config.dataset == "Potsdam":
...
print(
    "length of mapping assign and test dataloader %d"
...
if config.restart:
    if not choose_best:
        ...
        print("latest doesn't exist, using best")
        optimiser.load_state_dict(
            torch.load(os.path.join(config.out_dir, "best_optimiser.pytorch"))
        )
...
if config.restart:
    ...
    if not config.no_pre_eval:
        ...
        net.module.eval()
        acc, nmi, ari, masses = kmeans_segmentation_eval(
            config, net, mapping_assignment_test_dataloader
        )
...
for e_i in range(next_epoch, config.num_epochs):
    ...
    for b_i, tup in enumerate(dataloader):
        ...
        centre, other, adjacent = isola_set_patches(
            input_sz=config.input_sz, patch_side=config.isola_patch_side
        )
        adjacent_pred = net(img, centre=centre, other=other)

        loss = isola_loss(
            adjacent_pred, centre, other, adjacent, mask, verbose=config.verbose
        )

        if ((b_i % 100) == 0) or (e_i == next_epoch):
            print(
                "Model ind %d epoch %d batch: %d loss %f "
                "time %s"
    ...
    print("... metrics acc %f nmi %f ari %f time %s" % (acc, nmi, ari, datetime.now()))
    sys.stdout.flush()
