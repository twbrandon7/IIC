...
from code.utils.cluster.eval_metrics import _hungarian_match, _original_match, _acc
...
def _get_vectorised_sift_samples(archetype_config, dataloader):
    ...
    descs_all = np.zeros(
        (num_imgs_max, desc_side * desc_side, SIFT_DLEN), dtype=np.uint8
    )
    ...
    central_inds_h = (
        (np.arange(desc_side) * SIFT_STEP + (SIFT_STEP / 2))
        .reshape((desc_side, 1))
        .repeat(desc_side, axis=1)
    )
    central_inds_w = (
        (np.arange(desc_side) * SIFT_STEP + (SIFT_STEP / 2))
        .reshape((1, desc_side))
        .repeat(desc_side, axis=0)
    )
    central_inds_h, central_inds_w = central_inds_h.reshape(-1), central_inds_w.reshape(
        -1
    )
    ...
    for b_i, batch in enumerate(dataloader):
        if len(batch) == 3:  # test dataloader
            ...
            if labels_all is None:
                labels_all = np.zeros(
                    (num_imgs_max, desc_side * desc_side), dtype=np.int32
                )
        ...
        imgs = (imgs * 255.0).permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)
        masks = masks.cpu().numpy().astype(np.bool)

        curr_batch_sz, h, w, c = imgs.shape
        assert (
            h == archetype_config.input_sz
...
def _get_vectorised_colour_samples(archetype_config, dataloader):
    ...
    imgs_all = np.zeros(
        (num_imgs_max, img_sz, img_sz, archetype_config.in_channels), dtype=np.uint8
    )
...
def main():
    ...
    archetype_config_path = os.path.join(
        config.out_root, str(config.IID_model_ind), "config.pickle"
    )
    ...
    if config.test_code:
        ...
        chosen_inds = np.random.choice(
            samples.shape[0], size=num_samples_train, replace=False
        )
        samples = samples[chosen_inds, :]
        print(samples.shape)
    sys.stdout.flush()

    kmeans = MiniBatchKMeans(n_clusters=archetype_config.gt_k, verbose=1).fit(samples)
    ...
    assign_samples, assign_labels = sample_fn(
        archetype_config, mapping_assignment_dataloader
    )
