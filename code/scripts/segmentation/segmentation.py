...
from code.utils.segmentation.segmentation_eval import segmentation_eval
...
parser.add_argument(
    "--use_coarse_labels", default=False, action="store_true"
)  # COCO, Potsdam
parser.add_argument(
    "--fine_to_coarse_dict",
    type=str,  # COCO
...
parser.add_argument(
    "--include_things_labels", default=False, action="store_true"
)  # COCO
parser.add_argument("--incl_animal_things", default=False, action="store_true")  # COCO
...
parser.add_argument("--use_uncollapsed_loss", default=False, action="store_true")
...
parser.add_argument(
    "--out_root", type=str, default="/scratch/shared/slow/xuji/iid_private"
)
...
parser.add_argument("--pre_scale_all", default=False, action="store_true")
...
parser.add_argument("--use_random_affine", default=False, action="store_true")  # new
parser.add_argument("--aff_min_rot", type=float, default=-30.0)  # degrees
parser.add_argument("--aff_max_rot", type=float, default=30.0)  # degrees
parser.add_argument("--aff_min_shear", type=float, default=-10.0)  # degrees
parser.add_argument("--aff_max_shear", type=float, default=10.0)  # degrees
...
for e_i in range(next_epoch, config.num_epochs):
    ...
    for tup in itertools.izip(*iterators):
        ...
        all_img1 = (
            torch.zeros(config.batch_sz, pre_channels, config.input_sz, config.input_sz)
            .to(torch.float32)
            .cuda()
        )
        all_img2 = (
        ...
        all_affine2_to_1 = torch.zeros(config.batch_sz, 2, 3).to(torch.float32).cuda()
        all_mask_img1 = (
            torch.zeros(config.batch_sz, config.input_sz, config.input_sz)
            .to(torch.float32)
            .cuda()
        )
        ...
        if not (curr_batch_sz == config.dataloader_batch_sz) and (e_i == next_epoch):
        ...
        if not config.no_sobel:
            all_img1 = sobel_process(
                all_img1, config.include_rgb, using_IR=config.using_IR
            )
            all_img2 = sobel_process(
                all_img2, config.include_rgb, using_IR=config.using_IR
            )
        ...
        if ((b_i % 100) == 0) or (e_i == next_epoch):
            print(
                "Model ind %d epoch %d batch: %d avg loss %f avg loss no lamb %f "
                "time %s"
                % (
                    config.model_ind,
                    e_i,
