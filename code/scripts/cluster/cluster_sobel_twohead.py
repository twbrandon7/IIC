...
if config.restart:
    ...
    if config.select_sub_head_on_loss:
        sub_head = get_subhead_using_loss(
            config, dataloaders_head_B, net, sobel=True, lamb=config.lamb
        )
    _ = cluster_eval(
        config,
        net,
...
for e_i in range(next_epoch, config.num_epochs):
        ...
        for head_i_epoch in range(head_epochs[head]):
            ...
            for tup in itertools.izip(*iterators):
                ...
                if ((b_i % 100) == 0) or (e_i == next_epoch and b_i < 10):
                    print(
                        "Model ind %d epoch %d head %s head_i_epoch %d batch %d: avg "
    ...
    if config.double_eval:
        ...
        axarr[7].set_title(
            "double eval acc (avg), top: %f" % max(config.double_eval_avg_subhead_acc)
        )
