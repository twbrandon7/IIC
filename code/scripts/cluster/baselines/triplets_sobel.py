...
parser.add_argument(
    "--include_rgb", dest="include_rgb", default=False, action="store_true"
)
...
parser.add_argument(
    "--fluid_warp", dest="fluid_warp", default=False, action="store_true"
)
parser.add_argument(
    "--rand_crop_szs_tf", type=int, nargs="+", default=[]
)  # only used if fluid warp true
parser.add_argument(
    "--rot_val", type=float, default=0.0
)  # only used if fluid warp true
...
if config.restart:
    ...
    _ = triplets_eval(config, net, dataloader_test=dataloader_test, sobel=True)
...
for e_i in range(next_epoch, config.num_epochs):
    ...
    is_best = triplets_eval(config, net, dataloader_test=dataloader_test, sobel=True)
