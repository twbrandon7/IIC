...
parser.add_argument("--kmeans_on_features", default=False, action="store_true")

# transforms
# used for "positive" sample
parser.add_argument("--demean", dest="demean", default=False, action="store_true")
parser.add_argument(
    "--per_img_demean", dest="per_img_demean", default=False, action="store_true"
)
...
parser.add_argument(
    "--crop_other", dest="crop_other", default=False, action="store_true"
)
...
parser.add_argument(
    "--tf2_crop_szs", type=int, nargs="+", default=[84]
)  # allow diff crop for imgs_tf
parser.add_argument(
    "--tf3_crop_diff", dest="tf3_crop_diff", default=False, action="store_true"
)
parser.add_argument("--tf3_crop_sz", type=int, default=0)
parser.add_argument("--input_sz", type=int, default=96)

parser.add_argument("--rot_val", type=float, default=0.0)
parser.add_argument(
    "--always_rot", dest="always_rot", default=False, action="store_true"
)
...
dataloader_original, dataloader_positive, dataloader_negative, dataloader_test = (
    make_triplets_data(config)
)

train_dataloaders = [dataloader_original, dataloader_positive, dataloader_negative]
...
if config.restart:
    ...
    _ = triplets_eval(config, net, dataloader_test=dataloader_test, sobel=False)
...
for e_i in range(next_epoch, config.num_epochs):
    ...
    is_best = triplets_eval(config, net, dataloader_test=dataloader_test, sobel=False)
