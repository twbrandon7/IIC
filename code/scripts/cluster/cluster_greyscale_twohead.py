...
parser.add_argument("--dataset_root", type=str, default="/scratch/local/ssd/xuji/MNIST")
...
parser.add_argument(
    "--restart_from_best", dest="restart_from_best", default=False, action="store_true"
)
...
def train(render_count=-1):
    ...
    if config.restart:
        ...
        if config.select_sub_head_on_loss:
            sub_head = get_subhead_using_loss(
                config, dataloaders_head_B, net, sobel=False, lamb=config.lamb_B
            )
        _ = cluster_eval(
            config,
            net,
    ...
    fig, axarr = plt.subplots(
        6 + 2 * int(config.double_eval), sharex=False, figsize=(20, 20)
    )
    ...
    for e_i in range(next_epoch, config.num_epochs):
                ...
                for tup in zip(*iterators):
                    ...
                    for i in range(config.num_sub_heads):
                        loss, loss_no_lamb = IID_loss(
                            x_outs[i], x_tf_outs[i], lamb=lamb
                        )
        ...
        if config.double_eval:
            ...
            axarr[6].set_title(
                "double eval acc (best), top: %f" % max(config.double_eval_acc)
            )

            axarr[7].clear()
            axarr[7].plot(config.double_eval_avg_subhead_acc)
            axarr[7].set_title(
                "double eval acc (avg)), top: %f"
