...
from code.utils.cluster.transforms import sobel_process, sobel_make_transforms
...
def main():
    ...
    parser.add_argument("--penultimate_features", default=False, action="store_true")
    ...
    parser.add_argument("--restart_new_model_ind", default=False, action="store_true")
    ...
    reloaded_config_path = os.path.join(
        os.path.join(config.out_root, str(config.old_model_ind)), "config.pickle"
    )
    ...
    tf1, tf2, tf3 = sobel_make_transforms(
        old_config,
    ...
    train_data = dataset_class(
        root=old_config.dataset_root, transform=tf2, split="train"  # also could use tf1
    )
    ...
    test_data = dataset_class(
        root=old_config.dataset_root, transform=None, split="test"
    )
    test_data = TenCropAndFinish(
        test_data, input_sz=old_config.input_sz, include_rgb=old_config.include_rgb
    )
    ...
    opt_trunk = torch.optim.Adam(net.module.trunk.parameters(), lr=config.trunk_lr)
    opt_head = torch.optim.Adam(net.module.head.parameters(), lr=(config.head_lr))
    ...
    if not config.restart:
        ...
        print(
            "pre: model %d old model %d, acc %f time %s"
    ...
    for e_i in range(start_epoch, config.num_epochs):
        ...
        if e_i in config.lr_schedule:
            print(
                "e_i %d, multiplying lr for opt trunk and head by %f"
                % (e_i, config.lr_mult)
            )
