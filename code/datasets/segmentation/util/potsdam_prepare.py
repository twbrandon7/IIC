...
def main():
    ...
    for img_path in sorted(glob.glob(SOURCE_IMGS_DIR + "/*.tif")):
        ...
        split_and_save_imgs(
            img, next_index, names=save_names, cut=400, rescale=0.5, dir=out_dir_imgs
        )
        ...
        if os.path.isfile(gt_path):
            ...
            split_and_save_gts(
                gt, next_index, names=save_names, cut=400, rescale=0.5, dir=out_dir_gt
            )
...
def split_and_save_gts(gt, next_index, names, cut, rescale, dir):
    ...
    flat_gt = cv2.resize(
        flat_gt, dsize=None, fx=rescale, fy=rescale, interpolation=cv2.INTER_NEAREST
    )
    cut = int(cut * rescale)
