...
class DoerschDataset(data.Dataset):
    def __init__(self, config, base_dataset):
        ...
        if config.include_rgb and self.base_dataset.purpose == "train":
            ...
            self.stddev_fname = os.path.join(
                config.doersch_stats, "%s_stats.pickle" % config.dataset
            )
            ...
            print(
                "created Doersch dataset wrapping %s, stddev %s"
                % (config.dataset, self.stddev)
            )
            sysout.flush()
    ...
    def __getitem__(self, index):
        ...
        if self.base_dataset.purpose == "test":
            ...
            stddev = float(self.stddev[remaining_channel] / 100.0)
            noise = torch.zeros(
                (2, self.input_sz, self.input_sz), dtype=torch.float32
            ).cuda()
    ...
    def make_stats_file(self):
        ...
        pixels = np.zeros(
            (num_imgs * self.input_sz * self.input_sz, 3), dtype=np.float32
        )
        count = 0
